...
extension LayoutPropertyX {
    func equal(to otherAnchor: Anchor, multiplier: CGFloat = 0) {
        if let anchor = otherAnchor as? NSLayoutXAxisAnchor {
            anchor.constraint(equalToSystemSpacingAfter: anchor, multiplier: multiplier).isActive = true
        } else {
            fatalError("Não pode fazer o que quer")
        }
    }
}
